...
class DDQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        if random.random() > epsilon:
            state_tensor = torch.tensor(state, device = self.device).reshape(1, -1)#shape: [1, 8]
            with torch.no_grad():
                actions = self._behavior_net(state_tensor)
            return torch.max(actions, dim = 1)[1].item()#pick argmax index
        else:
    ...
    def _update_behavior_network(self, gamma):
        ...
        with torch.no_grad():
            q_next_idx = torch.max(self._target_net(next_state), dim = 1)[1].long().reshape(-1,1)
            q_next = self._target_net(next_state).gather(dim=1,index = q_next_idx)
